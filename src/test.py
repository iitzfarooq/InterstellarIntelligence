...
def get_noise_value(x, y):
    ...
    noise_value = (
        simplex.noise2(x / 50, y / 50)
        + 0.5 * simplex.noise2(x / 30, y / 30)
        + 0.25 * simplex.noise2(x / 10, y / 10)
    )
...
def draw_clouds(radius, cloud_density):
    ...
    adjusted_radius = min(
        radius * (cloud_density / 100), planet_radius
    )  # Ensure cloud radius doesn't exceed planet radius
    ...
    for y in range(int(center_y - adjusted_radius), int(center_y + adjusted_radius), 5):
        for x in range(
            int(center_x - adjusted_radius), int(center_x + adjusted_radius), 5
        ):
            distance = math.sqrt((x - center_x) ** 2 + (y - center_y) ** 2)
            if distance <= adjusted_radius:
                noise_value = simplex.noise2(
                    (x + cloud_noise_offset) / 80, (y + cloud_noise_offset) / 80
                )
...
def draw_dynamic_planet(variables):
    ...
    center_x, center_y = SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2
    planet_radius = (
        min(SCREEN_WIDTH, SCREEN_HEIGHT) // 3
    )  # Ensure planet fits within screen
    ...
    for y in range(center_y - planet_radius, center_y + planet_radius, 3):
        for x in range(
            center_x - planet_radius, center_x + planet_radius, 3
        ):  # Reduced step size for smoothness
            ...
            if distance_from_center <= planet_radius:
                ...
                pygame.draw.rect(
                    screen, terrain_color, (x, y, 3, 3)
                )  # Smaller rectangles for smoothness
...
def draw_horizontal_bar(x, y, width, value, label):
    ...
    pygame.draw.rect(
        screen, (224, 180, 74), (x, y, bar_value, bar_height), border_radius=3
    )
    text = font.render(f"{label}: {value:.2f}", True, WHITE)
...
while running:
    ...
    plants_density = max(
        0, min(100, dependent_variables.get("plants_density", 0))
    )  # subtract pollution to make it darker based on pollution levels
